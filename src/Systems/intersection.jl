...
function Overseer.update(::Intersector, m::AbstractLedger)
        ...
        @inbounds for (gen, intersections) in potential_intersections
            ...
            for (dist, trial, intersection) in new_intersections
                if dist > p.mindist * generation_distances[gen - 1]
                    Entity(m, m[Template][1], Generation(gen), trial, intersection)
                    n_added += 1
                end
            end
            if n_added != 0
    ...
    @debugv 2 "[STOP] Intersector"
end
# function Overseer.update(::Intersector, m::AbstractLedger)
#     lck = ReentrantLock()
#     gencomp = m[Generation]
#     maxgen = maximum(x->x.generation, gencomp, init=0)
#     for (p, es) in pools(m[IntersectionSearcher])
#         tot = 0
#         # Iterate over all entities that haven't been added to the intersection searcher
#         for e1 in Iterators.filter(x -> !(x in es), @entities_in(m, Results && Unique))
#             tote1 = 0 
#             # all entities that are lower in generation -> always intersect
#             @sync begin
#                 for e2 in Iterators.filter(x -> x.e != e1.e, @entities_in(m, Results && Unique))
#                     tstate = 0.5*(e1.state + e2.state)
#                     dist = max(p.mindist, p.mindist * Euclidean()(e1.state, e2.state))
#                     if !any(x -> Euclidean()(tstate, x.state) <= dist, m[Trial]) && !any(x -> Euclidean()(tstate, x.state) <= dist, m[Results])
#                         Entity(m, m[Template][e1], Trial(tstate, IntersectionMixed), Intersection(e1.e, e2.e), Generation(maxgen + 1))
#                         tote1 +=1
#                     end
#                 end
#             end
#             if tote1 != 0
#                 @debugv 2 "Total intersections starting at $(e1.e): $tote1"
#             end
#             m[IntersectionSearcher][e1] = es[1]
#             tot += tote1
#         end
#         @debugv 2 "Total intersections added for pool $(pool(m[IntersectionSearcher], es[1])): $tot"
#     end
# end
...
function Overseer.update(::RandomTrialGenerator, m::AbstractLedger)
    ...
    curgen = maximum(x->x.generation, @entities_in(m, RandomSearcher && Generation), init=1)
    # Wait until all intersections based on random generation have been finished
    if any(x -> m[Generation][x].generation == curgen && !(x in m[Results]), @entities_in(m[Intersection])) || any(x -> !(x in m[Results]), @entities_in(m[RandomSearcher] && m[Trial]))
        @debugv 2 "[STOP] RandomSearcher"
        return
    end

    # Set Generation to the maximum one considering the intersections
    maxgen = maximum(x->x.generation, m[Generation], init=1)
    template = m[Template][rand_search_e]
    ...
    for i = 1:nsearchers
        e = Entity(m, rand_trial(str, norb, nelec), Generation(maxgen + 1), template)
        if Hybrid in m && length(m[Hybrid]) != 0
            m[e] = Hybrid()
        end
        m[RandomSearcher][e] = rand_search_e
    end
